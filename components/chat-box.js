class ChatBox extends Component {
  ...
  _onAuthedMessageReceive({ detail }) {
    ...
    fetch("https://olivib.herokuapp.com/api/talk", {
      method: "POST",
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ message: detail.text })
    })
    ...
    .then((data) => {
      const bot = {
        text: data.message,
        time: new Date(),
        toChat: this._authedUserId,
        sender: this.activeChat.id,
      }
      this.renderMessage(bot, true);
      this.emit(APP_EVENTS.BOT_NEW_MESSAGE, bot);
    })
  }
