...
class ChatApp {
  ...
  initListeners() {
    ...
    this._componenets.chatBox.on(
      APP_EVENTS.BOT_NEW_MESSAGE,
      this._onBotNewMessages.bind(this)
    );
    this._componenets.chatBox.on(
  ...
  /**
   * this method fire when a new message from bot sent to a chat
   * @param detail
   * @private
   */
   _onBotNewMessages({ detail }) {
    // add sender property to message and
    // push it to the messages pool
    this._messages.push(detail);
  }
